      ...
      {editingTime && (
        <EditTimesModal
          ...
          onSave={(startedAt, completedAt) => {
            setData(prev => prev
              ? { ...prev, session: { ...prev.session, started_at: startedAt, completed_at: completedAt } }
              : prev)
      ...
      <div className="max-w-md mx-auto px-4 py-10 sm:py-16 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className="bg-surface/80 border border-edge rounded-xl p-4 flex flex-col items-center gap-1 text-center active:opacity-70"
          >
        ...
        <a
          href={buildGCalUrl({
            ...
            exercises: exercises.map(ex => ({
              name: ex.name,
              sets: setLogs.filter(l => l.exercise_template_id === ex.id),
            })),
