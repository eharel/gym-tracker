...
function CollapsibleBlock({ title, text }: { title: string; text: string }) {
  ...
  return (
    <div className="px-1">
      ...
      {open && (
        <p className="text-sm text-ink-secondary leading-relaxed pt-1 pb-2 pl-5">{text}</p>
      )}
    </div>
  )
}
...
export default function WorkoutScreen() {
  ...
  async function handleCompleteSession() {
    ...
    // Guard the one consequential tap: completing with unlogged working sets
    // is usually a misclick (it has happened) — ask only in that case
    const remaining = data.setLogs.filter(
      l => !skipped.has(l.exercise_template_id) &&
           (l.set_type === 'top' || l.set_type === 'working' || l.set_type === 'backoff' || l.set_type === 'amrap') &&
           !l.completed,
    ).length
    if (remaining > 0 && !confirm(
      `${remaining} working set${remaining === 1 ? '' : 's'} not logged — complete the session anyway?`,
    )) return
